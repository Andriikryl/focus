...
export const Gauge: React.FC<GaugeProps> = ({
  ...
  const defaultArcObject: DefaultArcObject = {
    innerRadius: 0,
    outerRadius: 0,
    startAngle: 0,
    endAngle: 0,
  ...
  const backgroundArc = arc<DefaultArcObject>()
    ...
    .endAngle(Math.PI / 2)
    .cornerRadius(1)(defaultArcObject);
  ...
  const filledArc = arc<DefaultArcObject>()
    .innerRadius(0.65)
    .outerRadius(1)
    .startAngle(-Math.PI / 2)
    .endAngle(angle)
  ...
  const gradientSteps: string[] = colorScale
    .ticks(10)
  ...
  return (
    <div style={{ textAlign: "center" }} className={styles.wrapper}>
      ...
      {!!units && (
        <div
          style={{ color: "#8b8ba7", lineHeight: "1.3em", fontWeight: "300" }}
        >
          {units}
        </div>
      )}
